      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          ...
          >
            <span
              aria-hidden
              className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-foreground font-display text-[15px] leading-none italic text-background"
            >
              {initial}
            </span>
        ...
        <DropdownMenuContent align="start" side="bottom" sideOffset={4} className="w-[244px]">
          ...
          {workspaces.map((w) => (
            ...
            >
              <span
                aria-hidden
                className="grid h-5 w-5 shrink-0 place-items-center rounded bg-foreground/10 font-mono text-[9px] uppercase text-foreground"
              >
                {w.name.slice(0, 1)}
              </span>
